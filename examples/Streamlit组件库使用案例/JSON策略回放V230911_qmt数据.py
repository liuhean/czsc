...
def show_trader(trader: CzscTrader, files):
    ...
    for freq in freqs:
        ...
        kline = KlineChart(n_rows=3, row_heights=(0.5, 0.3, 0.3), title="", width="100%", height=980)
        ...
        kline.add_sma(df, ma_seq=(5, 10, 20, 120, 240), row=1, visible=False, line_width=1)
    ...
    with st.sidebar:
        with st.form(key="my_form_replay"):
            ...
            sdt, edt = date_range_picker(
                "回放起止日期", default_start=pd.to_datetime("2019-01-01"), default_end=pd.to_datetime("2022-01-01")
            )
